...
#include <vector>
#include <algorithm>
LeaderboardController::LeaderboardController(PlatformInterface* p) {
...
void LeaderboardController::cleanUp() {
    ...
    for(int i = 0; i < NUM_HIGH_SCORES && i < nameScores.size(); i++) {
        ...
        for(ImageInfo* img : nameImages) {
            visualRenderer->renderImage(
                img,
                LBOARD_NAME_OFFSET_X + (j * LBOARD_CHAR_SIZE),
                LBOARD_NAME_OFFSET_Y - (i * LBOARD_CHAR_SIZE * LBOARD_ENTRIES_GAP)
            );
        ...
        for(ImageInfo* img : scoreImages) {
            visualRenderer->renderImage(
                img,
                LBOARD_SCORE_OFFSET_X + (j * LBOARD_CHAR_SIZE),
                LBOARD_SCORE_OFFSET_Y - (i * LBOARD_CHAR_SIZE * LBOARD_ENTRIES_GAP)
            );
...
std::list<ImageInfo*> LeaderboardController::constructName(std::string str) {
    ...
    for(char c : str) {
        ...
        name.push_back(resourceMan->makeImage(path, LBOARD_CHAR_SIZE, LBOARD_CHAR_SIZE, true));
    }
    return name;
}

std::list<ImageInfo*> LeaderboardController::constructScore(std::string str) {
    ...
    for(char c: str) {
        ...
        score.push_back(resourceMan->makeImage(path, LBOARD_CHAR_SIZE, LBOARD_CHAR_SIZE, true));
    }
    return score;
}
